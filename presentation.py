...
class Representor:
    ...
    def show_size_options(self, name, base_font, big_font, base_step, offset, text_color, color, input_color):
        print_text(self.screen, 'Hi ' + name + '!', big_font, text_color, color, offset)
        offset += big_font.size('A')[1] + 10
        ret_offset = offset
    ...
    def check_size(self, name, event, base_font, big_font, base_step, big_step, offset, text_color, color, input_color, num_of_bottles):
        print(offset)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if x < left or x > left + step * (size_options // 2):
                return num_of_bottles
            if y < offset + base_step + big_step or y > big_step + offset + 2 * base_step + height:
                return num_of_bottles

            left_offset = (x - left) // step

            if y > big_step + offset + height + base_step:
                ret = left_offset + size_options // 2
                self.show_size_options(name, base_font, big_font, base_step, offset, text_color, color, input_color)
                print_text_on_rect(self.screen, str(start + ret), pygame.Rect(int(left + step * left_offset),
                                                                              offset + 2 * base_step + big_step,
    ...
    def start_screen(self):
        ...
        default_color = (153, 51, 255)
        active_color = (153, 153, 255)
        ...
        print_text_on_rect(self.screen, input_text, input_rect, base_font, text_color, curr_color)
    ...
    def new_game_window(self, name):
        ...
        while True:
            for event in pygame.event.get():
                ...
                num_of_bottles = self.check_size(name, event, base_font, big_font, base_step, big_step, offset,
                                                 text_color, color,
    ...
    def congratulations(self):
        congratulation_surf = pygame.Surface((2 * self.width // 3, self.height // 3))
        congratulation_color = (255, 0, 127)
        congratulation_surf.fill(congratulation_color)
        base_font = pygame.font.Font(None, 40)
        text_color = (255, 255, 255)
        offset = 10
        print_text(congratulation_surf, 'Congratulations!', base_font, text_color, congratulation_color, 10)
        offset += base_font.size('A')[1] + 10
        confirm_rect = pygame.Rect(self.width // 6, offset, 200, 35)
        input_color = (255, 255, 102)
        print_text_on_rect(congratulation_surf, 'Go on!', confirm_rect, base_font, text_color, input_color)

        self.screen.blit(congratulation_surf, (self.width // 6, self.height // 3, 2 * self.width // 3, self.height // 3))
        pygame.display.flip()
        confirm_rect.x += self.width // 6
        confirm_rect.y += self.height // 3
        return confirm_rect
